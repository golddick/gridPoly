...
import { boardIndex } from "@/lib/game/board";
import { tileCurrentValue, getColorGroupStatus } from "@/lib/game/engine";
import { BET_MULTIPLIERS } from "@/lib/game/types";
import type { GameState } from "@/lib/game/types";
...
export default function TileInfoPanel({
  ...
  const currentValue = tileCurrentValue(game, tileId);
  const owner = ts.ownerPlayerId ? game.players[ts.ownerPlayerId] : null;
  const groupStatus = getColorGroupStatus(game, tileId, myPlayerId);
        ...
        <div className="mt-2 space-y-1 text-cream/70">
          ...
          {groupStatus && (
            <Row
              label="Color group"
              value={
                owner && myPlayerId === owner.id
                  ? `${groupStatus.ownedByPlayer}/${groupStatus.total} owned — ${groupStatus.ownsAll ? "can build" : "need the full set to build"}`
                  : `${groupStatus.total} properties in this set`
              }
            />
          )}
        </div>
