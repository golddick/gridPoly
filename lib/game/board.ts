...
/** How many consecutive same-type properties/estates form one color group — must own the whole group to build. */
export const GROUP_SIZE = 2;

/** Distinct accent colors cycled per color group, so groups are visually readable on the board (like real Monopoly's color sets). No blue, per the brand palette. */
export const GROUP_PALETTE = ["#8B5A2B", "#2E8B57", "#B23A2E", "#E0A72E", "#7A4FB5", "#4A9B7F", "#C0392B", "#9C6B30"];
...
export function generateBoard(boardSizeInput: number): BoardTile[] {
  ...
  for (let position = 0; position < total; position++) {
    ...
    const isGrouped = type === "property" || type === "estate";
    const groupIndex = Math.floor(occurrence / GROUP_SIZE);
    const paletteOffset = type === "estate" ? 4 : 0;
    ...
    tiles.push({
      ...
      colorGroup: isGrouped ? `${type}-group-${groupIndex}` : undefined,
      groupColor: isGrouped ? GROUP_PALETTE[(groupIndex + paletteOffset) % GROUP_PALETTE.length] : undefined,
    });
  }
