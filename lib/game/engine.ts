...
export function createInitialGameState(
  ...
  players: { id: string; userId: string; username: string; pieceId: string; isBot?: boolean; botDifficulty?: BotDifficulty }[]
): GameState {
...
/**
 * Whoever the game is currently waiting on: the active bidder in an auction, the
 * player facing a landing decision, or the current turn player. Exported so the
 * room server, the bot policy, and the tests all share one source of truth for
 * "who acts next" — the seam the bot driver hooks into.
 */
export function whoMustAct(state: GameState): string | null {
