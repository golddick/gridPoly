...
export interface BoardTile {
  ...
  colorGroup?: string; // property/estate only — must own every tile in the same group to build
  groupColor?: string; // display accent for the group (property/estate only)
}
...
/**
 * Difficulty tier for an in-memory bot player. Declared here (not in bot.ts) so
 * PlayerState can carry it without the types module depending on the bot policy.
 */
